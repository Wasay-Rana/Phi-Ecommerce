# ePHI — Ecommerce Storefront

A Next.js 16 (App Router) storefront for ePHI, a Pakistan-based tech and STEM/maker products brand. Built with TypeScript, React 19, Tailwind CSS v4 (theme driven by CSS variables), Zustand for cart state, and EmailJS for order notification emails.
...
The `template_3wc1jbp` body is the HTML in [`docs/emailjs-order-template.html`](docs/emailjs-order-template.html) — a card-style layout using the merge fields `src/lib/emailjs.ts` actually sends: `order_id`, `customer_name`, `customer_phone`, `customer_address`, `order_items`, `order_subtotal`, `order_shipping`, `order_total`, `payment_method`, `order_notes`. If you ever need to recreate or edit the template, paste that file's contents into EmailJS's template body editor (Email Templates → `template_3wc1jbp` → Content). The live template on EmailJS still has the old "Phi" wording baked in from before the rename — update it there too.

## Product taxonomy

Products are split into two niches (`src/types/product.ts`, `src/data/products.ts`):

- **Tech** — Audio, Charging, Wearables, Phone Accessories, Desk Setup, Smart Home
- **STEM & Maker** — 3D-Printed Lamps, Decor, STEM Kits

Navigation (mega-menu in the header, home page niche cards, shop filters, footer) is all generated from `niches` / `categoriesByNiche` in `src/data/products.ts` — add a category there and it appears everywhere automatically.

## Theming

All colors live as CSS custom properties in `src/app/globals.css` under `:root` (prefixed `--raw-*`), mapped into Tailwind v4's `@theme` block in the same file to generate utilities (`bg-accent`, `text-primary`, `border-border`, etc.). Tailwind v4 is CSS-first — there is no `tailwind.config.ts`. To re-theme the site, edit the hex values in the `:root` block in `globals.css` only.
...
- `src/data/products.ts` — sample product catalog and niche/category taxonomy (replace with a real data source when ready)
